...
def get_picked_point(action):
    ...
    # key_point_index = np.array([0, 24, 49, 99, 9902, 9924, 9947, 9997]) ## change it for the first experiment!!!!!!!!!!!!
    key_point_index = np.array([0, 24, 99, 9902, 9924, 9997])
    # key_point_index = np.linspace(0, 100*100 -1, 100*100)
...
def get_reward(initial_pos, current_pos):
    ...
    pos_group_c = current_pos[fold_group_c]
    pos_group_d = current_pos[fold_group_d]

    pos_group_b_init = initial_pos[fold_group_b]
    pos_group_d_init = initial_pos[fold_group_d]
...
def run_task(vv, log_dir, exp_name):
    ...
    saved_action_path = './data/side_fold/action_traj_1.npy'
    action_seq_ = np.load(saved_action_path)
    num_actions_ = len(action_seq_[:,0])

    for k in range(num_actions_):
        current_action = action_seq_[k]
        obs, reward, _, info = env.step(current_action)
        print(reward)

    saved_action_path = './data/reflect_fold/action_traj_7.npy'
    action_seq_ = np.load(saved_action_path)
    num_actions_ = len(action_seq[:,0])
    ...
    env.action_tool.reset_action_space()
    #################################################
...
def main():
    ...
    # # data/cem/simple/20220827224547/traj/action_traj_1.npy
    # 4 24
    # 15 8
    path_to_action = 'data/cem/simple/20220923171414/traj/action_traj_1.npy'
