...
for i in range(m):
    j = i + 1
    temp_name = osp.join(current_path, '../data/cem/original/20220904225250/data/cost_his_' + str(j) +'.npy')
...
# original_data = np.vstack((ori_cost_his_flatten, ori_num_iter)).T
# # ori_panda = pd.DataFrame(original_data, columns=['cost', 'iteration'])
# ori_panda = pd.DataFrame({'cost':ori_cost_his_flatten,
#                           'iteration':ori_num_iter,
#                           'method': "original"})
...
for i in range(m):
    j = i +1
    temp_name = osp.join(current_path, '../data/cem/simple/20220903212630/data/cost_his_' + str(j) +'.npy')
...
# cost_his = np.hstack([simp_cost_his_flatten, ori_cost_his_flatten])
# num_iter = np.hstack([simp_num_iter, ori_num_iter])
# methods = np.hstack([simp_methods, ori_methods])

# df = pd.DataFrame({'Cost':cost_his, 
#                     'Iteration':num_iter, 
#                     'Methods': methods})

# compare_panda = pd.concat([ori_panda, simp_panda], axis = 0)
# print(compare_panda)
# print(compare_panda[compare_panda.index.duplicated()])
# compare_panda = compare_panda[~compare_panda.index.duplicated()]

# compare_panda = compare_panda[['method', 'iteration', 'cost']]
# # Save the order of the arrival months
# order = compare_panda['iteration']
# # Pivot the table to turn it into wide-form
# compare_panda_wide = compare_panda.pivot_table(index='iteration', columns='method', values='cost')
# # Reindex the DataFrame with the `order` variable to keep the same order of months as before
# compare_panda_wide = compare_panda_wide.reindex(order, axis=0)
# print(compare_panda_wide)
###############################################################################
m = 10
n = 30
...
for i in range(m):
    j = i +1
    temp_name = osp.join(current_path, '../data/random/cem/20230207155513/data/cost_his_' + str(j) +'.npy')
...
for i in range(m):
    j = i +1
    temp_name = osp.join(current_path, '../data/cem/hand_crafted/20220906193324/data/reward_his_' + str(j) +'.npy')
...
# fig, ax = plt.subplots()
# # plt.hold(1)fig_cost_ori = sns.lineplot(x = 'Iteration', y ='Cost', hue = 'Methods', data = df, ci = 95, lw=2, palette=['#0000FF', '#FF6103', '#71C671', '#000000']) #
...
# plt.xticks(np.arange(1,n,3), fontsize = 30)
