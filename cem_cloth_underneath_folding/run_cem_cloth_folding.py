...
def run_task(vv, log_dir, exp_name):
    ...
    for i in range(vv['test_episodes']):
        ...
        while j < env.horizon:
            ...
            time_start = time.time()
            action, cost_his= policy.get_action(initial_state) # obs
            time_end = time.time()
...
def main():
    ...
    parser.add_argument('--env_name', default='ClothUnderneathFold') #RopeConfiguration RopeFlatten ClothFold
    ...
    key_point_idx = np.array([0, 24, 99, 9902, 9924, 9997])
    #get_rand_edge_idx(width= 100, height = 100, size = 6) #
    # print(key_point_idx)
