...
class CEMPolicy(object):
    ...
    def reset(self, prev_sol = None, mean_coeff = 2, var_coeff = 4):
    ...
    def get_action(self, state=None, ):
        ...
        soln, cost_his= self.optimizer.obtain_solution(cur_state = env_state, mean_coeff = self.mean_coeff, var_coeff = self.var_coeff, init_mean = self.prev_sol)
        soln = soln.reshape([-1, self.action_dim])
        if self.use_mpc:
