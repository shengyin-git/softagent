...
def get_cost(args):
    ...
    if env is None:
        ...
        env = env_class(**env_kwargs)
        print('Child env created!')
    # env.reset(config_id=init_state['config_id'], initial_state = init_state)
    env.reset(config_id=init_state['config_id'])
...
class ParallelRolloutWorker(object):
    ...
    def cost_function(self, init_state, action_trajs):
        ...
        #     # args = dict(init_state = init_statreward_hise, action_trajs = splitted_action_trajs[i], env_class = self.env_class, env_kwargs = self.env_kwargs)
        #     ret = get_cost(**args)
