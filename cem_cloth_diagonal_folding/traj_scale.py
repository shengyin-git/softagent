import sys
sys.path.insert(1, '/home/shengyin/OneDrive/Lab_Ubun_Projects/softagent_diagonal_folding_cem/')
...
def run_task(vv, log_dir, exp_name):
    ...
    obs = env.reset()
    # reset action space & key point idx
    # env.get_goal_key_pos(goal_pos = [], key_idx = [])
    ...
    ## rest the cloth
    for i in range(10):
        obs, reward, _, info = env.step(np.array([0,0,0,1]), record_continuous_video=True, img_size=img_size)
        frames.extend(info['flex_env_recorded_frames'])
        # time.sleep(1)
        # temp_image = env.get_image(img_size, img_size)
        # show_picture(img_size, img_size, temp_image[:,:,0].flatten(), temp_image[:,:,1].flatten(), temp_image[:,:,2].flatten(), save_path = logdir)
    ...
    if osp.exists(save_dir):           
        ...
        np.save(save_dir + 'particle_pos_his_' + ts +'.npy', particle_pos)
    else:
        os.makedirs(save_dir, exist_ok=True)
        # np.save(save_dir + 'picker_pos_his_' + ts +'.npy', picker_pos_his)
        # np.save(save_dir + 'picker_pos_his_new_' + ts +'.npy', picker_pos_his_new)
...
def main():
    ...
    # ./data/post_process/original/ 
    parser.add_argument('--log_dir', default='./data/post_process/simp_model/')
    ...
    # # 15 8
    # path_to_action = 'data/cem/original/20220902221347/traj/action_traj_2.npy'
    # path_to_action = 'data/simp/cem/20230208165413/traj/action_traj_1.npy'
    path_to_action = 'data/simp_second/cem/20230208172326/traj/action_traj_1.npy'
    # path_to_action = './data/simp_ori/cem/20221207143256/traj/action_traj_1.npy'
    ...
    reorder_key_point_idx, nodes_idx, node_flat_positions, node_folded_positions, edges = \
            pre_data_process(flatten_pos_path = './data/simp_model/particle_pos_ini.npy',\
                        ...
                        second_model_path = './data/simp_model/second_simped_model.npy')
