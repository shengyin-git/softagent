...
def run_task(vv, log_dir, exp_name, prev_sol=None):
    print(vv['test_num'])
    ...
    for i in range(vv['test_episodes']):
        ...
        obs = env.reset()
        policy.reset(prev_sol = prev_sol)
...
def main():
    ...
    parser.add_argument('--env_name', default='ClothDiagonalFold') #RopeConfiguration RopeFlatten ClothFold
    ...
    key_point_idx = np.array([8010,0,89,8099])
    # print(key_point_idx)
    ...
    # prev_sol_path = './data/simp/cem/traj/action_traj_0.npy'
    # prev_sol = np.array([np.load(prev_sol_path).flatten()])
    # print(prev_sol)
