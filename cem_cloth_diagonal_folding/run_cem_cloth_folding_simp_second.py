...
def run_task(vv, log_dir, exp_name, prev_sol, prev_var):
    print(vv['test_num'])
    ...
    for i in range(vv['test_episodes']):
        ...
        obs = env.reset()
        policy.reset(prev_sol = prev_sol, prev_var = prev_var)
...
def main():
    ...
    #
    parser.add_argument('--env_kwargs_horizon', default=14, type=int) #15
    ...
    prev_sol_path = './data/simp/cem/20230208165413/traj/action_traj_1.npy'
    prev_sol = np.array([np.load(prev_sol_path).flatten()]) #None #np.array([np.load(prev_sol_path).flatten()])
    prev_var_path = './data/simp/cem/20230208165413/traj/action_var_1.npy'
    prev_var = np.array([np.load(prev_var_path).flatten()]) #None #np.array([np.load(prev_sol_path).flatten()])
    print(prev_sol)
    print(prev_var)

    run_task(args.__dict__, args.log_dir, args.exp_name, prev_sol, prev_var)
