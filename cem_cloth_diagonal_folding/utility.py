...
def pre_data_process(flatten_pos_path = './data/simp_model/particle_pos_ini.npy',\
                        ...
                        key_point_idx_path='./data/simp_model/key_point_idx.npy',\
    ...
    # key_point_idx = np.load(osp.join(current_path, key_point_idx_path), allow_pickle=True)
    edges = simp_model['simp_edges']
    nodes = simp_model['simp_nodes'] # should be different from key points in the future
    key_point_idx = nodes
    ...
    if second_model_path is not None:
        _reorder_key_point_idx, _nodes_idx, _node_flat_positions, _node_folded_positions, _edges = \
            pre_data_process(flatten_pos_path = flatten_pos_path,\
                            folded_pos_path = folded_pos_path,\
        ...
        for i in range(num_key_points):
            distance = np.sqrt(np.sum(np.asarray(node_flat_positions[nodes_idx[i]] - _node_flat_positions[_nodes_idx])**2, axis = 1))
            # print(distance)
            index_distance = np.transpose(np.vstack((_reorder_key_point_idx, _nodes_idx, distance))) 
            index_distance = index_distance[np.argsort(index_distance[:,2])]
            reorder_key_point_idx[i] = index_distance[0,0]
            nodes_idx[i] = np.int64(index_distance[0,1]) # this one remains undecided
            # print(reorder_key_point_idx[i])
            # print(nodes_idx[i])
        # reorder_key_point_idx = _reorder_key_point_idx
        # nodes_idx = _nodes_idx

        node_flat_positions = _node_flat_positions
        node_folded_positions = _node_folded_positions
        edges = _edges

    return reorder_key_point_idx, nodes_idx, node_flat_positions, node_folded_positions, np.array(edges)
