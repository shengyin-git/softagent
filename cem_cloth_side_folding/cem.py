...
class CEMOptimizer(object):
    ...
    def obtain_solution(self, cur_state, init_mean=None, init_var=None):
        mean = (self.ub + self.lb) / 2. if init_mean is None else init_mean
        var = (self.ub - self.lb) / 8. if init_var is None else init_var # 4
        ...
        while (t < self.max_iters):  # and np.max(var) > self.epsilon:
            ...
            t += 1
        sol, solvar = best_sol, 0
        return sol, cost_his

class CEMPolicy(object):
    """ Use the ground truth dynamics to optimize a trajectory of actions. """

    def __init__(self, env, env_class, env_kwargs, use_mpc, plan_horizon, max_iters, population_size, num_elites, prev_sol=None):
    ...
    #     return soln[0], cost_his # only return the first action in mpc
    ...
    def get_action(self, state):
        ...
        soln, cost_his= self.optimizer.obtain_solution(cur_state = env_state, init_mean = self.prev_sol)
        soln = soln.reshape([-1, self.action_dim])
        if self.use_mpc:
        ...
        self.env.set_state(env_state)  # Recover the environment
        print("cem finished planning!")
        return soln, cost_his # this gives all actions in mpc
