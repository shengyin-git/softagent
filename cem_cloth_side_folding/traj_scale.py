import sys
sys.path.insert(1, '/home/shengyin/OneDrive/Lab_Ubun_Projects/softagent_cloth_folding_cem/')
...
def get_picked_point(action):
    ...
    key_point_index = np.array([4, 49, 97, 9902, 9947, 9996]) 
    # key_point_index = np.linspace(0,9999,10000) 
...
def plotting_actions(particle_pos_his, picker_pos_his_new, action_sequence, save_path):
    ...
    dia_sim = np.linalg.norm(particle_pos_his[0,0,:] - particle_pos_his[0,99,:])/99
    len_sim = dia_sim * 100
...
def run_task(vv, log_dir, exp_name):
    ...
    if get_initial_image:
        temp_image = env.get_initial_image(img_size, img_size)
        ...
        if osp.exists(save_dir):
            np.save(save_dir + 'flatten_ini_pos.npy', particle_pos)
        else:
            os.makedirs(save_dir, exist_ok=True)
            np.save(save_dir + 'flatten_ini_pos.npy', particle_pos)
        # return
    ...
    if get_folded_data:
        ...
        temp_image = env.get_goal_image(img_size, img_size)
        show_picture(img_size, img_size, temp_image[:,:,0].flatten(), temp_image[:,:,1].flatten(), temp_image[:,:,2].flatten(), save_path = logdir)
        particle_pos = np.array(pyflex.get_positions()).reshape(-1, 4)[:, :3]
    ...
    # with open('./softgym/cached_initial_states/cloth_fold_init_states.pkl', "rb") as handle:
    ...
    for i in range(num_actions):
        ...
        # state_info.append(env.get_new_state())
    ...
    # idx_max = np.argmax(reward_his)
    # print(idx_max)
    # init_state = state_info[idx_max]
    # cached_init_states[0]['particle_pos']=init_state['particle_pos']
    # print((init_state['particle_pos'].reshape(-1,4)[:,3]==1).all())    
    # print(init_state['particle_pos'].reshape(-1,4)[:,3])
    # np.save()
    # with open('./softgym/cached_initial_states/cloth_fold_init_states_1.pkl', 'wb') as handle:
    #         pickle.dump((cached_configs, cached_init_states), handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def main():
    ...
    parser.add_argument('--env_name', default='ClothFold') #RopeConfiguration RopeFlatten ClothFold
    parser.add_argument('--log_dir', default='./data/post_process/simple/')
    ...
    # 20220828114814 4 36
    # 20220829220627 8 16
    # path_to_action = 'data/cem/simple/20220920202052/traj/action_traj_1.npy'
    # path_to_action = './data/simp/cem/traj/action_traj_0.npy'
    path_to_action = './data/simp_ori/cem/20221207220641/traj/action_traj_1.npy'
